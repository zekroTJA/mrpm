...
RELEASE_URL="https://api.github.com/repos/zekrotja/$APP_NAME/releases/latest"
DEFAULT_INSTALL_PATH="/usr/local/bin/$APP_NAME"
...
main() {
    ...
    if [ -z "${INSTALL_PATH}" ]; then
        INSTALL_PATH=$(command -v "$APP_NAME" || true)
    fi
    ...
    if [ -d "$INSTALL_PATH" ]; then
        confirm "You have given a path as install path ($INSTALL_PATH); do you want to install $APP_NAME to $INSTALL_PATH/$APP_NAME?"
    ...
    download_tmp_file="$download_tmp_dir/$APP_NAME"
    ...
    info "$APP_NAME has been installed successfully to $INSTALL_PATH!"
}
